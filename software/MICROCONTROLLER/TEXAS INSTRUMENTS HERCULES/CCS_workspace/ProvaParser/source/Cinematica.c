...
bool calcAngles(const Point2D _point, bool _isAlpha)
{
  if (_isAlpha)
  {
    alpha.mainAngle = setAngle( calcPhase(_point) + acos(( pow(length1, 2) + pow( calcModule(_point), 2) - pow( length2, 2)) / ( 2 * length1 * calcModule(_point))));
    alpha.secondAngle = setAngle(acos((pow(length1, 2) - pow(calcModule(_point), 2) + pow(length2, 2)) / (2 * (float)length1 * length2)) + alpha.mainAngle - PI);
  }
  else
  {
    beta.secondAngle = setAngle(calcPhase(_point) + acos(( pow(length1, 2) + pow( calcModule(_point), 2) - pow( length2, 2)) / ( 2 * length1 * calcModule(_point))));
    beta.mainAngle = setAngle(acos((pow(length1, 2) - pow(calcModule(_point), 2) + pow(length2, 2)) / (2 * length1 * length2)) + beta.secondAngle - PI);
  }
  return true;
}
...
float calcJointSpeed(Vector2D _velocity, Point2D _point, bool _isAlpha)
{
  ...
  if (_isAlpha)
  {
      if(alpha.secondAngle!=0){
    q2 = pointV2.y - pointV2.x * tan(alpha.secondAngle + PI_2);
    point12.x = q2 / (tan(alpha.mainAngle + PI_2) - tan(alpha.secondAngle + PI_2));
  ...
  {
      if(beta.mainAngle!=0){
    q2 = pointV2.y - pointV2.x * tan(beta.mainAngle + PI_2);
    point12.x = q2 / (tan(beta.secondAngle + PI_2) - tan(beta.mainAngle + PI_2));
