...
{
    *_alpha =setAngle(atan2(_point.y,_point.x)+acos(( pow(LENGTH1, 2) + pow( calcModule(_point), 2) - pow( LENGTH2, 2)) / ( 2 * LENGTH1 * calcModule(_point))))/PI*25000;
    *_beta = setAngle(acos((pow(LENGTH1, 2) - pow(calcModule(_point), 2) + pow(LENGTH2, 2)) / (2 * (float)LENGTH1 * LENGTH2)))/PI*12500;
}
