...
bool calcMotorAngles(const Point2D _point, bool _isAlpha)
{
  if (_isAlpha)
  {
    ...
    betaAbs.mainAngle=alpha.mainAngle+alpha.secondAngle-PI;
  }
  else
  {
    calcAngles(_point,&beta.secondAngle,&beta.mainAngle);
    betaAbs.secondAngle=beta.secondAngle+beta.mainAngle-PI;
  }
  return true;
}

bool calcAngles(const Point2D _point, float* _alpha, float* _beta)
{
    *_alpha =setAngle(atan2(_point.y,_point.x)+acos(( pow(LENGTH1, 2) + pow( calcModule(_point), 2) - pow( LENGTH2, 2)) / ( 2 * LENGTH1 * calcModule(_point))));
    *_beta = setAngle(acos((pow(LENGTH1, 2) - pow(calcModule(_point), 2) + pow(LENGTH2, 2)) / (2 * (float)LENGTH1 * LENGTH2)));
    return true;
}
