...
{
  return sqrtf((_point1.x - _point2.x)*(_point1.x - _point2.x) + (_point1.y - _point2.y)*(_point1.y - _point2.y));
}

bool pointTransport(Point2D _startPoint, float _length, float _phase, Point2D* _endPoint)
{
  if (fitAngle(_phase) == fitAngle(M_PI_2) || fitAngle(_phase) == fitAngle(3 * M_PI_2))
  {
    _endPoint->x = _startPoint.x;
    _endPoint->y = _startPoint.y + _length * sin(_phase);
  }
  else if (fitAngle(_phase) == fitAngle(M_PI))
  {
